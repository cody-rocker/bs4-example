...
def parse_args():
    ...
    for argument in sys.argv:
        ...
        if argument in ['-d', '--debug']:
            DEBUG = True
            args['debug'] = True
            Log.debug('parse_args', 'debug output enabled')

        if argument in ['-vv', '--verbose']:
            DEBUG = True
            VERBOSE = True
            args['verbose'] = True
            Log.debug('parse_args', 'verbose output enabled')
...
def parse_feed_to_dict(feed, _dict=None):
    ...
    videos = []
    for video in feed.find_all('div', class_='yt-lockup-content'):
        videos.append(parse_entity(video))

    videos = set(videos)
    results[section_label.text] = videos
...
def scrape_query_results(query):
    ...
    videos = []
    for video in query_results.find_all('div', class_='yt-lockup-content'):
        videos.append(parse_entity(video))

    return {'Search Results': set(videos)}
...
if __name__ == '__main__':
    args = parse_args()  # check command line arguments to set script parameters

    # perform script operation
    if args['query'] is not None:
        results = scrape_query_results(args['query'])
    else:
        results = scrape_homepage()
